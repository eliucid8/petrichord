...
def update_project(destination, libs, include_vs=False):
    ...
    for f in f_to_cp:
        ...
        if os.path.isdir(sname):
            ...
            rewrite_replace(dname, './libDaisy/', libs + '/libDaisy')
            rewrite_replace(dname, './DaisySP/', libs + '/DaisySP')
...
def create_from_template(destination, board, libs, include_vs = False):
    ...
    rec_rewrite_replace(destination, "./libDaisy/", libdaisy_dir)

    daisysp_dir=libs + "/DaisySP/"
    rec_rewrite_replace(destination, "./DaisySP/", daisysp_dir)
